...
**값 두 개만 넣으면 켜집니다.** 안 넣으면 `내 계정`에 캘린더 줄이 나타나지 않습니다.

### 6-1. 값 두 개 찾기

둘 다 **공개돼도 되는 값**입니다. 캘린더 앱의 브라우저 번들에 이미 들어 있고,
실제 보안 경계는 캘린더 쪽 `firestore.rules`(내 계정 아래만 읽고 씀)입니다.

찾는 곳은 둘 중 편한 쪽:

**(A) Cloudflare 대시보드** — Workers & Pages → `dada-calendar` →
Settings → Environment variables. 거기 있는

- `VITE_FIREBASE_API_KEY` (`AIza…`로 시작합니다)
- `VITE_FIREBASE_PROJECT_ID`

**(B) Firebase 콘솔** — 프로젝트 설정(톱니) → 일반 → 내 앱 → SDK 설정 및 구성.
거기 `apiKey`와 `projectId`가 있습니다.

### 6-2. 넣기

`index.html` 위쪽 `const CAL = {` 안의 두 줄을 채웁니다.
...
푸시하면 Cloudflare Pages가 알아서 다시 배포합니다.
...
- **캘린더 줄이 안 보인다** → 두 값이 비어 있습니다. 6-2를 다시 봅니다.
